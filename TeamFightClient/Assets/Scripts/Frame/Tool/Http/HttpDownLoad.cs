...
namespace ZW.Frame
{
    ...
    public class HttpDownLoad
    {
		...
		//public HttpDownLoadManager.DownLoadData downloadData;
        /// <summary>
        /// 下载方法(断点续传)
        /// </summary>
        ...
        public void DownLoad(string url, string savePath, Action callBack, string savename = null)
        {
            ...
            thread = new Thread(delegate () {
				try{
					...
					Debug.Log("Down load file :" + savename);
	                if (File.Exists(savePath + "\\" + savename))
	                {
	                    Debug.Log("File already exists " + savePath + "\\" + savename);
	                    progress = 1;
	                    isDone = true;
	                ...
	                if (progress == 1)
	                {
	                    ...
	                    isDone = true;
	                    if (callBack != null) callBack.Invoke();
						Debug.Log("download file done: " + savename);
	                }
				}
				catch(Exception e)
				{
					Debug.LogError("error occur when download file: " + savename + "excetion is: " + e.ToString());
				}
            });
            //开启子线程
